# Const values
special_strings = ["one", "two", "three", "four", "five", "six", "seven", "eight", "nine"]
...
def get_calibration_in_line(line):
    ...
    for i in range(len(line)):
        ...
        for j in range(len(special_strings)):
            if line[i:].find(special_strings[j]) == 0:
                first_calibration, last_calibration = get_new_calibration_pair(first_calibration, last_calibration, j + 1)

    return first_calibration * 10 + last_calibration


# Resolve the day01-part2 problem
def main():
    result = 0
